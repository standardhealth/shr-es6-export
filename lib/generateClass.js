  ...
  if (fhirElement.type[0].code === 'Reference') {
    ...
    cw.bl('if (!mappedResources[entryId])', () => {
      ...
      cw.bl('if (referencedEntry)', () => {
        ...
        cw.ln(`mappedResources[entryId] = FHIRHelper.createInstanceFromFHIR('${shrType}', referencedEntry['resource'], /* TODO: */ null, shrId, allEntries, mappedResources, referencesOut);`);
      });
    });
    ...
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      if (shrType.isPrimitive && !isExtension) {
        ...
        rhs = `FHIRHelper.createInstanceFromFHIR('${shrType.fqn}', ${fhirPathString}, 'Extension', shrId, allEntries, mappedResources, referencesOut, ${isExtension})`;
      }
      if (isRef) {
        rhs = `FHIRHelper.createReference( ${rhs}, referencesOut)`;
      }
