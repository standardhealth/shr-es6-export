...
function generateFactory(namespaces) {
  ...
  cw.ln(`import { getNamespaceAndName, getNamespaceAndNameFromFHIR } from './json-helper';`);
    ...
    .bl('export default class ObjectFactory', () => {
      ...
      cw.blComment(() => {
        ...
        .bl('static createInstanceFromFHIR(fhir, type, allEntries=[], mappedResources={}, referencesOut=[], asExtension=false)', () => {
          ...
          for (const ns of namespaces) {
            const factory = factoryName(ns.namespace);
            cw.ln(`case '${ns.namespace}': return ${factory}.createInstanceFromFHIR(fhir, type, allEntries, mappedResources, referencesOut, asExtension);`);
          }
          cw.ln(`default: throw new Error(\`Unsupported namespace: \${namespace}\`);`)
            .ln('}');
