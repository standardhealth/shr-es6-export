...
class ES6Exporter {
  ...
  collectValueSets(fhir) {
    ...
    cw.ln('export const valueSet = (url) => VS_MAP[url] || [];')
      .ln()
      .ln('const VS_MAP =')
      .indent();
    ...
    // write the map out line-by-line  ;; TODO: figure out how to get code lists on a single line

    JSON.stringify(vsMap, null, 2).split('\n').forEach(line => cw.ln(line));
