...
class ES6Exporter {
  ...
  export() {
    ...
    es6Defs['fhirGeneralPurposeTypeMapping.js'] = this.generateFhirGeneralPurposeMapping(this._fhir);
  ...
  generateFhirGeneralPurposeMapping(fhir) {
    const cw = new CodeWriter();

    cw.ln(`export const FHIR_GENERAL_PURPOSE_TYPES = {`)
      .indent();

    // TODO: is there a generic way to fetch these types and fields?
    // TODO: do these change across r2/3/4?
    const fhirFields = {
    ...
    const allFhirProfiles = [...fhir._noDiffProfiles, ...fhir.profiles];

    for (const type in fhirFields) {
      const fields = fhirFields[type];

      debugger;

      // TODO: this is insufficient as multiple profiles might map to the right type
      // and I don't see an obvious way to pick which one is correct
      // (other than matching on name, which feels brittle)
      const matchingProfile = allFhirProfiles.find(p => p.type === type);

      cw.ln(`'${type}': {`)
        .indent()
        .ln(`fields: [${fields.map(stringify).join(', ')}],`);

      if (matchingProfile) {
        const fqn = matchingProfile.id.replace(/-/g, '.');
        const lastDot = fqn.lastIndexOf('.');
        const namespace = fqn.slice(0, lastDot);
        const elementName = fqn.slice(lastDot + 1);

        cw.ln(`mapsTo: { namespace: '${namespace}', elementName: '${elementName}' }`);
      } else {
        // if there is no matching profile, that may cause issues down the line if those objects are used
        // but this is not the place to address that
        cw.ln(`mapsTo: {}`)
      }
    ...
    // add reference at the end specially because it's unique
    cw.ln(`// references may not be the best fit here but it works`);
    cw.bl(`'Reference': `, () => {
      cw.ln(`fields: ['reference', 'type', 'identifier', 'display'],`)
        .ln(`mapsTo: { namespace: '', elementName: 'Reference' }`);
    });
