...
function generateClass(def, specs, fhir) {
  ...
  if (fhirProfile || fhirExtension || def.value) {
    imports.push('createInstanceFromFHIR', 'createReference', 'uuid');
  }
...
function childElementsOf(parent, allElements) {
  ...
  for (const element of allElements) {
    ...
    if (element.path == parent.path && !parent.slicing) { // if the parent isn't sliced then we know it is a slice. TODO: this might break on nested slices!!
...
function writeFromFhir(def, specs, fhir, fhirProfile, fhirExtension, cw) {
  ...
  if (def.isEntry) {
    cw.ln(`inst.entryInfo = createInstanceFromFHIR('shr.base.Entry', {});`); // do it this way so we don't have to import Entry
    cw.ln(`inst.entryInfo.shrId = createInstanceFromFHIR('shr.base.ShrId', shrId);`);
    cw.ln(`inst.entryInfo.entryId = createInstanceFromFHIR('shr.base.EntryId', fhir['id'] || uuid());`); // re-use the FHIR id if it exists, otherwise generate a new uuid

    // copied from writeToJson above --- should this URL be configurable?
    const url = `http://standardhealthrecord.org/spec/${def.identifier.namespace.replace('.', '/')}/${className(def.identifier.name)}`;
    cw.ln(`inst.entryInfo.entryType = createInstanceFromFHIR('shr.base.EntryType', '${url}');`);
  }
    ...
    for (const mapping of element.mapping) {
      ...
      if(mapping.map === '<Value>'){
        ...
        if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
          generateFromFHIRAssignment(def.value.card.isList, def.value instanceof RefValue, element, fhirElementPath, [], 'value', fhirProfile, null, cw);
        } else {
        ...
        for (let i = 0 ; i < mapping.fieldChain.length ; i++) {
          ...
          if (i == mapping.fieldChain.length - 1) { // if it's the last field in the chain
            generateFromFHIRAssignment(field.card.isList, field instanceof RefValue, element, fhirElementPath, mapping.fieldMapPath, shrElementPath, fhirProfile, slicing, cw);
          } else {
            ...
            let rhs = `createInstanceFromFHIR('${field.effectiveIdentifier.fqn}', {}, shrId)`;
            if (field instanceof RefValue) {
              rhs = `createReference( ${rhs}, referencesOut)`;
            }
          ...
          if (field.card.isList) {
            ...
            const pushLine = `${elementHierarchy[0].shrElementPath}.push(${shrElementPath});`;
            if (element.type[0].code === 'Reference') {
              // if it's a reference, wrap it in a null check in case it was missing
              cw.bl(`if (${shrElementPath} != null)`, () => {
                cw.ln(pushLine);
              });
    ...
    fhirExtension.differential.element.forEach( (element, i) => {
      if(element.path.startsWith('Extension.value') && element.path !== 'Extension.value[x]'){
        ...
        cw.bl(`if (${varName} != null)`, () => {
          cw.ln(`inst.${methodName} = createInstanceFromFHIR('${instance}', ${varName}, shrId, allEntries, mappedResources, referencesOut, true);`); // asExtension = true here, false(default value) everywhere else
        });
      }
    });
  });
  ...
  cw.bl(`if (!asExtension && fhir != null)`, () => {
    if (def.value.effectiveIdentifier != null) {
      if (def.value.effectiveIdentifier.isPrimitive) {
        ...
        cw.ln(`inst.value = createInstanceFromFHIR('${shrType}', fhir, shrId, allEntries, mappedResources, referencesOut);`);
      }
    } else {
      ...
      cw.ln(`inst.value = createInstanceFromFHIR(null, fhir, shrId, allEntries, mappedResources, referencesOut);`);
    }
  });
}
        ...
        if (sliceGroup.slicing.discriminator.some( d => d.path == element.path )) {
          ...
          if (isDstu2) {
            if (element.type[0].profile) {
              url = element.type[0].profile[0]; // TODO: can there be more than 1 type or profile? or be missing?
            }
          } else {
  ...
  return slicing.map(discriminator => {
    ...
    for (let i = 0 ; i < pathArray.length ; i++) {
      ...
      if (fullPath.endsWith('.extension.url')) {
        // extension.url doesn't always exist in the profiles. TODO: should it be?
        currElement = {}; // HACK just to get past the list check below
      } else {
        currElement = allFhirElements.find(e => e.path === fullPath);
      }
 ...
 * @param {boolean} cardIsList - whether the element cardinality is a "list" (aka > 1)
 * @param {boolean} isRef - whether the element is an SHR ref() type
 ...
 * @param {CodeWriter} cw - the CodeWriter that is writing the file for this element
 */
function generateFromFHIRAssignment(cardIsList, isRef, fhirElement, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  ...
  if(fhirElementPath.endsWith(`['extension']`)) {
    ...
    cw.bl(`if (match != null)`, () => {
      ...
      let rhs = `createInstanceFromFHIR('${shrType}', match, shrId, allEntries, mappedResources, referencesOut, true)`; // asExtension = true here, false(default value) everywhere else
      if (isRef) {
        rhs = `createReference( ${rhs}, referencesOut)`;
      }
      cw.ln(`${shrElementPath} = ${rhs};`);
    });
    ...
    cw.bl('if (!mappedResources[refID])', () => {
      ...
      cw.bl('if (referencedEntry)', () => {
        ...
        cw.ln(`mappedResources[refID] = createInstanceFromFHIR('${shrType}', referencedEntry['resource'], shrId, allEntries, mappedResources, referencesOut);`);
      });
    });

    let rhs = 'mappedResources[refID]';
    if (isRef) {
      rhs = `createReference( ${rhs}, referencesOut)`;
    }

    cw.ln(`${dec}${shrElementPath} = ${rhs};`); // if it failed to map, this could be undefined, which is fine
  } else {
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      const shrMapping = shrElementMapping[shrElementMapping.length - 1];
      const shrType = fhirMappingToIdentifier(shrMapping).fqn;
      let rhs = `createInstanceFromFHIR('${shrType}', ${fhirPathString}, shrId, allEntries, mappedResources, referencesOut)`;
      if (isRef) {
        rhs = `createReference( ${rhs}, referencesOut)`;
      }
