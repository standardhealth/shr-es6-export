      ...
      if(mapping !== undefined){
        if(mapping.map === '<Value>'){
          ...
          if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
            generateFromFHIRAssignment(def.value.card.isList, baseIsList, 1, element.path, [], 'value', fhirProfile, cw);
          } else {
          ...
          if(fieldMapPath){
            ...
            if(field !== undefined){
              ...
              if (!alreadyMappedElements.has(classMethodChain)) {
                ...
                generateFromFHIRAssignment(field.card.isList, baseIsList, constraintsLength, element.path, fieldMapPath, classMethodChain, fhirProfile, cw);
 ...
 * @param {boolean} baseCardIsList whether the element is "based" on an element whose cardinality is a list (> 1)
 * @param {Number} constraintsLength the number of constraints on the element; used to determine whether there are more contstrained elements than cardinalities
 * @param {string} fhirElementPath the FHIR element that the data should be taken from  (in FQN notation)
 * @param {string} shrElementPath the SHR element that the data should be put into (in FQN notation)
 * @param {Object} fhirProfile the FHIR profile the element comes from
 * @param {CodeWriter} cw the CodeWriter that is writing the file for this element
 */
function generateFromFHIRAssignment(cardIsList, baseCardIsList, constraintsLength, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, cw) {
  
  if (fhirElementPath == 'Patient.active')
  {
    debugger;
  }
  ...
  cw.bl(`if (${check})`, () => {
    ...
    while(pathArray.length > 0){
      ...
      const shrMapping = shrElementMapping.shift(); // TODO: why was this undefined?
    ...
    // if(fhirElementPath.split('.')[1] === 'extension'){
    //   cw.ln(`inst['extension'] = inst['extension'] || [];`);
    //   cw.ln(`inst['extension'].push(typeof this.${shrElementPath}.toFHIR === 'function' ? this.${shrElementPath}.toFHIR(true) : this.${shrElementPath});`);
    // } else {
      if (cardIsList || baseCardIsList) {
        cw.ln(`inst.${shrPathString} = inst.${shrPathString} || [];`);
        if (cardIsList) {
          cw.ln(`inst.${shrPathString} = inst.${shrPathString}.concat(fhir.${shrElementPath}.map(f => typeof f.toFHIR === 'function' ? f.toFHIR() : f));`);
        } else {
          cw.ln(`inst.${shrPathString}.push(typeof fhir.${shrElementPath}.toFHIR === 'function' ? fhir.${shrElementPath}.toFHIR() : fhir.${shrElementPath});`);
        }
      } else {
        // If we have to map our elements to an array higher up in the profile
        if (containerProfileArray) {
