...
class ES6Exporter {
  ...
  defToES6(def) {
    ...
    if (def.basedOn.length) {
      ...
      if (def.basedOn[0] instanceof TBD) {
        ...
        superClass = sanitizeIdentifierName(def.basedOn[0].name);
    ...
    cw.bl(`class ${className}${superClass ? ` extends ${superClass}` : ''}`, () => {
      this.generateClassBody(def, cw);
    });
    cw.ln(`export default ${className};`);
    return cw.toString();
  }
  ...
  generateClassBody(def, cw) {
    ...
    if (def.isEntry) {
      this.writeGetterAndSetter(cw, 'shr.base.Entry', 'entryInfo', 'entry information', '_entryInfo', 'Entry');
    }

    if (def.value) {
      if (def.value instanceof ChoiceValue) {
        this.writeGetterAndSetter(cw, def.value, 'value');
        ...
        if (!def.value.identifier.isValueKeyWord) {
          ...
          this.writeGetterAndSetter(cw, def.value, 'value', `value (aliases ${symbol})`, `_${symbol}`);
          this.writeGetterAndSetter(cw, def.value);
        }
      } else {
    ...
    for (const field of def.fields) {
      this.writeGetterAndSetter(cw, field);
    }
  }

  writeGetterAndSetter(cw, formalDefOrName, publicSymbol, descriptiveName, privateSymbol, typeName) {
