...
function generateClass(def, specs, fhir) {
  ...
  if (fhirProfile || fhirExtension || def.value) {
    imports.push('createInstanceFromFHIR', 'createReference');
  }
...
function generateClassBody(def, specs, fhir, fhirProfile, fhirExtension, cw) {
  ...
  }).bl('static fromFHIR(fhir, allEntries=[], mappedResources={}, referencesOut=[], asExtension=false)', () => writeFromFhir(def, specs, fhir, fhirProfile, fhirExtension, cw))
    .ln();
}
      ...
      if(mapping.map === '<Value>'){
        ...
        for (let i = 0 ; i < mapping.fieldChain.length ; i++) {
          ...
          if (i == mapping.fieldChain.length - 1) { // if it's the last field in the chain
            ...
            let rhs = `createInstanceFromFHIR('${field.effectiveIdentifier.fqn}', {})`;
    ...
    fhirExtension.differential.element.forEach( (element, i) => {
      if(element.path.startsWith('Extension.value') && element.path !== 'Extension.value[x]'){
        ...
        cw.bl(`if (${varName} != null)`, () => {
          cw.ln(`inst.${methodName} = createInstanceFromFHIR('${instance}', ${varName}, allEntries, mappedResources, referencesOut, true);`); // asExtension = true here, false(default value) everywhere else
        });
      }
    });
  });
  ...
  cw.bl(`if (!asExtension && fhir != null)`, () => {
    if (def.value.effectiveIdentifier != null) {
      if (def.value.effectiveIdentifier.isPrimitive) {
        ...
        cw.ln(`inst.value = createInstanceFromFHIR('${shrType}', fhir, allEntries, mappedResources, referencesOut);`);
      }
    } else {
      ...
      cw.ln(`inst.value = createInstanceFromFHIR(null, fhir, allEntries, mappedResources, referencesOut);`);
    }
  });
}
...
function generateFromFHIRAssignment(cardIsList, isRef, fhirElement, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  ...
  if(fhirElementPath.endsWith(`['extension']`)) {
    ...
    cw.bl(`if (match != null)`, () => {
      ...
      let rhs = `createInstanceFromFHIR('${shrType}', match, allEntries, mappedResources, referencesOut, true)`; // asExtension = true here, false(default value) everywhere else
      if (isRef) {
        rhs = `createReference( ${rhs}, referencesOut)`;
      }
    ...
    cw.bl('if (!mappedResources[refID])', () => {
      ...
      cw.bl('if (referencedEntry)', () => {
        ...
        cw.ln(`mappedResources[refID] = createInstanceFromFHIR('${shrType}', referencedEntry['resource'], allEntries, mappedResources, referencesOut);`);
      });
    });
    ...
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      const shrMapping = shrElementMapping[shrElementMapping.length - 1];
      const shrType = fhirMappingToIdentifier(shrMapping).fqn;
      let rhs = `createInstanceFromFHIR('${shrType}', ${fhirPathString}, allEntries, mappedResources, referencesOut)`;
      if (isRef) {
        rhs = `createReference( ${rhs}, referencesOut)`;
      }
