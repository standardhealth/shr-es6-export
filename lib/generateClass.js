...
function writeToFhir(def, fhir, cw) {
  ...
  if(fhirProfile !== undefined){
      ...
      if(mapping !== undefined){
        if(mapping.map === '<Value>'){
          if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
            generateFHIRAssigment(def.value.card, element.path, 'value', cw);
          } else {
          ...
          if(fieldMapPath){
            ...
            if(field !== undefined){
              generateFHIRAssigment(field.card, element.path, classMethodChain, cw);
            }
          }
        }
      }
    ...
    // No profile, not an entry, but it has a value so we can resolve the value
    // This will recursively call toFHIR on the values until we reach a primitive
    cw.bl(`if (this.value != null)`, () => {
      if (def.value.card.isList) {
...
function generateFHIRAssigment(card, fhirElementPath, valueString, cw) {
  ...
  cw.bl(`if (${check})`, () => {
    ...
    if (card.isList) {
      cw.ln(`inst${pathString} = this.${valueString}.map(f => typeof f.toFHIR === 'function' ? f.toFHIR() : f.toFHIR());`);
    } else {
