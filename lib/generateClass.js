    ...
    cw.bl(`if (fhir != null)`, () => {
      if (def.value.identifier != null) {
        if (def.value.identifier.isPrimitive) {
          // primitive value so we can set it directly
          cw.ln(`inst.setValue(fhir);`);
        } else {
          const shrType = def.value.identifier.name;
          cw.ln(`inst.setValue( ${shrType}.fromFHIR(match) );`);
        }
      } else if (def.value.options != null) {
        // figure out which one it is?

        def.value.options.forEach(o => {
          const shrType = o.identifier.name;
          cw.ln(`// could be a ${shrType}`);
        });
      }
    });
  }
  ...
  cw.bl(`if (${check})`, () => {
    ...
    if(fhirElementPath.split('.')[1] === 'extension'){
      ...
      cw.bl(`if (match != null)`, () => {
          ...
          const shrType = fhirMappingToIdentifier(shrMapping).name;
          cw.ln(`inst.${shrElementPath} = ${shrType}.fromFHIR(match);`);
          // TODO: can an extension be an array?
      });
      ...
      if (cardIsList) {
        ...
        cw.ln(`inst.${shrPathString} = inst.${shrPathString}.concat(fhir${fhirPathString}.map(f => ${shrType}.fromFHIR(f)));`);
      } else if (baseCardIsList) {
        const shrMapping = shrElementMapping[shrElementMapping.length - 1];
        const shrType = fhirMappingToIdentifier(shrMapping).name;
        // not a list; only keep the first one
        cw.ln(`inst.${shrPathString} = ${shrType}.fromFHIR(fhir${fhirPathString}[0]);`);
        ...
        if (containerProfileArray) {
          ...
          if (shrElementPath == 'value') {
            ...
            const shrMapping = shrElementMapping[shrElementMapping.length - 1];
            const shrType = fhirMappingToIdentifier(shrMapping).name;
            cw.ln(`inst.${shrElementPath} = ${shrType}.fromFHIR(fhir${fhirPathString});`);
          }
        }
      }
    }
