...
const { sanitizeName, className, upperCaseFirst } = require('./common.js');
 ...
 * @param {boolean} isDstu2 - Whether the elements are DSTU2; 2/3 have different slicing structures.
 * @returns {Array} elements from allElements that are children of `parent`. (parent itself is not returned in the list)
 */
function childElementsOf(parent, allElements, isDstu2) {
  ...
  for (const element of allElements) {
    ...
    if (sliceName(parent) && sliceName(element) != sliceName(parent)) {
      break;
    }
...
function writeFromFhir(def, specs, fhir, fhirProfile, fhirExtension, cw) {
  ...
  cw.ln('return {result: inst, references};');
}
...
function writeFromFhirProfile(def, specs, fhir, fhirProfile, cw) {
  ...
  //    didOpenBlock: {boolean},  //
  //    fhirBasePath: {string},
  //    shrElementPath: {string},  //
  //    shrBasePath: {string},  //
  // }
  ...
  for (const element of allFhirElements) {
    ...
    while (elementHierarchy.length > 0 &&
      (!element.path.startsWith(elementHierarchy[0].element.path)
        || (element.path === elementHierarchy[0].element.path && sliceName(elementHierarchy[0].element) && sliceName(elementHierarchy[0].element) != sliceName(element)))) { // slices are not parents of other slices
    ...
    const childElementsWithMapping =  childElementsOf(element, allFhirElements, isDstu2).filter(e => e.mapping.length > 0);
    ...
    if (element.mapping.length > 0 || childElementsWithMapping.length > 0) {
      ...
      if (fhirIsList) {
        ...
        if (slicing && sliceName(element) && slicing[sliceName(element)]) {
          ...
          const discriminatorCheck = buildDiscriminatorCheck(element, slicing[sliceName(element)], fhirElementPath, allFhirElements);
          ...
          basePath = fhirElementPath + '[0]'; // TODO - other ways to pick?
    ...
    for (const mapping of element.mapping) {
      ...
      if(mapping.map === '<Value>'){
        ...
        if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
          generateFromFHIRAssignment(def.value.card.isList, element, fhirElementPath, [], 'value', fhirProfile, null, cw);
        } else {
        ...
        for (let i = 0 ; i < mapping.fieldChain.length ; i++) {
          ...
          if (i == mapping.fieldChain.length - 1) { // if it's the last field in the chain
            generateFromFHIRAssignment(field.card.isList, element, fhirElementPath, mapping.fieldMapPath, shrElementPath, fhirProfile, slicing, cw);
          } else {
            ...
            // TODO: merge this into the generateFromFHIRAssignment above
            ...
            cw.ln(`${dec}${shrElementPath} = ${nullCheck}createInstanceFromFHIR('${field.effectiveIdentifier.fqn}', {}).result;`);
          }
    ...
    fhirExtension.differential.element.forEach( (element, i) => {
      if(element.path.startsWith('Extension.value') && element.path !== 'Extension.value[x]'){
        ...
        cw.bl(`if (${varName} != null)`, () => {
          const retValObjName = varName + '_result';
          cw.ln(`const ${retValObjName} = createInstanceFromFHIR('${instance}', ${varName}, allEntries, mappedResources, true);`); // asExtension = true here, false(default value) everywhere else
          cw.ln(`inst.${methodName} = ${retValObjName}.result;`);
          cw.ln(`references.push(...(${retValObjName}.references));`);
        });
      }
    });
  });
  ...
  cw.bl(`if (!asExtension && fhir != null)`, () => {
    if (def.value.effectiveIdentifier != null) {
      if (def.value.effectiveIdentifier.isPrimitive) {
        ...
        cw.ln(`const valueResult = createInstanceFromFHIR('${shrType}', fhir, allEntries, mappedResources);`);
        cw.ln(`inst.value = valueResult.result;`);
        cw.ln(`references.push(...(valueResult.references));`);
      }
    } else {
      ...
      cw.ln(`const valueResult = createInstanceFromFHIR(null, fhir, allEntries, mappedResources);`);
      cw.ln(`inst.value = valueResult.result;`);
      cw.ln(`references.push(...(valueResult.references));`);
    }
  });
...
function preprocessSlicing(fhirProfile) {
  ...
  for (let element of fhirProfile.snapshot.element) {
    ...
    if (element.slicing && !element.path.endsWith('.extension')) { // TODO: extensions automatically slice on url, figure out what we need to do about that
      ...
      if (sliceNm) {
        ...
        sliceStack[0].currentSlice = sliceNm;
        sliceStack[0].elements[sliceNm] = [];
      }
...
function buildDiscriminatorCheck(element, slicing, fhirElementPath, allFhirElements) {
  return slicing.map(discriminator => {
    ...
    for (let i = 0 ; i < pathArray.length ; i++) {
      const v = pathArray[i];
      prev = prev + '.' + v;
      discCheck = (i == 0 ? '' : discCheck + ' && ') + bracketNotation(prev) + ' != null'; // don't add to the previous on index 0; there is no previous

      fullPath = fullPath + '.' + v; // TODO: is there a better way to handle this? we need both deconstructed and reconstructed paths.

      const currElement = allFhirElements.find(e => e.path === fullPath);
      ...
      if (currElement.max == '*' || Number(currElement.max) > 1 || ( currElement.base && (currElement.base.max == '*' || Number(currElement.base.max) > 1))) {
        ...
        discCheck = discCheck + ' && ' + bracketNotation(prev) + '.some(' + innerVar + ' => ' + buildDiscriminatorCheck(currElement, slicing, innerVar, allFhirElements) + ')';
      ...
      if (i == pathArray.length - 1) {
        ...
        if (discriminator.value) {
          ...
          // TODO: linter complains about double quoted strings
          discCheck = discCheck + ' && ' + bracketNotation(prev) + '.equals(' + JSON.stringify(discriminator.value) + ')';
        }
      }
  ...
  if(fieldMapPath){
    ...
    for(const pathElement of fieldMapPath) {
      ...
      if (!field) {
        ...
        console.error(`Original Element: ${def.identifier} ; full mapping: ${mapping} ; FHIR ID: ${element.id} Path: ${element.path}`); // TODO: DSTU2 doesn't have element.id
 ...
 * @param {CodeWriter} cw - the CodeWriter that is writing the file for this element
 */
function generateFromFHIRAssignment(cardIsList, fhirElement, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  let fhirPathString = bracketNotation(fhirElementPath);
  const retValObjName = createVariableName(shrElementPath + '_retVal'); // temporary storage for the return value of callng createInstanceFromFHIR
  ...
  if(fhirElementPath.endsWith(`['extension']`)) {
    ...
    cw.bl(`if (match != null)`, () => {
      ...
      cw.ln(`const ${retValObjName} = createInstanceFromFHIR('${shrType}', match, allEntries, mappedResources, true);`); // asExtension = true here, false(default value) everywhere else
      cw.ln(`${shrElementPath} = ${retValObjName}.result;`);
      cw.ln(`references.push(...(${retValObjName}.references));`);
    });
    ...
    cw.bl('if (!mappedResources[refID])', () => {
      ...
      cw.bl('if (referencedEntry)', () => {
        ...
        cw.ln(`const ${retValObjName} = createInstanceFromFHIR('${shrType}', referencedEntry['resource'], allEntries, mappedResources);`);
        cw.ln(`mappedResources[refID] = ${retValObjName}.result;`);
        cw.ln(`references.push(...(${retValObjName}.references));`);
      });
    });
    ...
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      const shrMapping = shrElementMapping[shrElementMapping.length - 1];
      const shrType = fhirMappingToIdentifier(shrMapping).fqn;
      cw.ln(`const ${retValObjName} = createInstanceFromFHIR('${shrType}', ${fhirPathString}, allEntries, mappedResources);`);
      cw.ln(`${dec}${shrElementPath} = ${retValObjName}.result;`);
      cw.ln(`references.push(...(${retValObjName}.references));`);
    }
  }
}
