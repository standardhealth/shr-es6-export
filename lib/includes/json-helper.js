 ...
 * @private
 */
export function createInstanceFromFHIR(key, value) {
  if (Array.isArray(value)) {
    return value.map(v => createInstanceFromFHIR(key, v));
  }
  if (typeof value === 'object') {
    // TODO: review these. are these relevant in the fromFHIR context?
    // if (value._ShrId && value._EntryId && value._EntryType) {
    //   // It's a reference, so just return the reference
    //   return new Reference(value._ShrId, value._EntryId, value._EntryType);
    // } else if (value.code && value.codeSystem) {
    ...
    return OBJECT_FACTORY.createInstanceFromFHIR(value, key);
  }
  return value;
}
