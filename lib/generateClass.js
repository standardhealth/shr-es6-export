          ...
          if(fieldMapPath){
            ...
            if(field !== undefined){
              ...
              if (!alreadyMappedElements.has(classMethodChain)) {
                ...
                generateFromFHIRAssignment(field.card.isList, baseIsList, constraintsLength, element.path, classMethodChain, fhirProfile, cw);
 ...
 * @param {string} valueString the SHR element that the data should be put into (in FQN notation)
 * @param {Object} fhirProfile the FHIR profile the element comes from
 * @param {CodeWriter} cw the CodeWriter that is writing the file for this element
 */
function generateFromFHIRAssignment(cardIsList, baseCardIsList, constraintsLength, fhirElementPath, valueString, fhirProfile, cw) {
  ...
  cw.bl(`if (${check})`, () => {
    ...
    // } else {
      if (cardIsList || baseCardIsList) {
        cw.ln(`inst${pathString} = inst${pathString} || [];`);
        if (cardIsList) {
          cw.ln(`inst${pathString} = inst${pathString}.concat(fhir.${valueString}.map(f => typeof f.toFHIR === 'function' ? f.toFHIR() : f));`);
        } else {
          cw.ln(`inst${pathString}.push(typeof fhir.${valueString}.toFHIR === 'function' ? fhir.${valueString}.toFHIR() : fhir.${valueString});`);
        }
      } else {
        // If we have to map our elements to an array higher up in the profile
        if (containerProfileArray) {
          // Iterate through each element at current level, and map it to the array element
          cw.bl(`fhir.${valueString}.forEach( (elem) => `, () => {
            cw.ln('let containerInst = {};');
            let containedPathString = '';
            // Iterate through each level between the array element and our current element, and make sure they're all initialized.
            containedPathArray.forEach(e => {
          ...
          cw.ln(`inst.${valueString} = fhir${pathString};`);
        }
      }
    //}
  });
