...
function generateClass(def, specs, fhir) {
  ...
  cw.bl(`class ${clazzName}${superClass ? ` extends ${superClass}` : ''}`, () => {
    generateClassBody(def, specs, fhir, cw);
  });
 ...
 * @private
 */
function generateClassBody(def, specs, fhir, cw) {
  cw.ln();
  ...
  cw.blComment( () => {
    ...
    .bl(`toFHIR(asExtension=false)`, () => writeToFhir(def, specs, fhir, cw))
    .ln();
  ...
  }).bl('static fromFHIR(fhir, asExtension=false)', () => writeFromFhir(def, specs, fhir, cw))
    .ln();
}
 ...
 * @private
 */
function writeFromFhir(def, specs, fhir, cw) {
        ...
        if(mapping.map === '<Value>'){
          ...
          if(fieldMapPath){
            ...
            for(const pathElement of fieldMapPath) {
              field = currDef.fields.find(f => f.identifier && f.identifier.equals(fhirMappingToIdentifier(pathElement)));

              if (!field) {
                break;
              }

              currDef = specs.dataElements.byNamespace(field.identifier.namespace).find(e =>  e.identifier.equals(field.identifier));
            }
 ...
 * @private
 */
function writeToFhir(def, specs, fhir, cw) {
