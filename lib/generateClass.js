...
function writeFromFhirProfile(def, specs, fhir, fhirProfile, cw) {
  ...
  const isQuantity = allFhirElements && allFhirElements.length > 0 && allFhirElements[0].path === 'Quantity';
  for (const element of allFhirElements) {
    ...
    if (isQuantity && element.path === 'Quantity.code' && element.mapping.some(m => m.identity === 'shr' && m.map === '<shr.core.Units>')) {
      cw.ln(`let coding = {};`);
