...
function writeFromFhirProfile(def, specs, fhir, fhirProfile, cw) {
  ...
  for (const element of allFhirElements) {
    ...
    const childElementsWithMapping = allFhirElements.filter(e => e !== element && e.path.startsWith(element.path + '.') && e.mapping && e.mapping.find(m => m.identity === 'shr'));
    ...
    if (elementMappings.length > 0 || childElementsWithMapping.length > 0) {
      ...
      if (fhirIsList) {
        ...
        if (elementMappings.length > 0) {
          const {field} = getFieldAndMethodChain(elementMappings[0], def, specs, element);
          if (field) {
            shrIsList = field.card.isList;
          }
        } else {
          // check children
          const firstChildMapping = childElementsWithMapping[0].mapping.find(m => m.identity === 'shr');
          const {field} = getFieldAndMethodChain(firstChildMapping.map, def, specs, element);
          if (field) {
            shrIsList = field.card.isList;
          }
        }
    ...
    for (const mapping of elementMappings) {
      ...
      if(mapping === '<Value>'){
        ...
        const {field, classMethodChain, fieldMapPath} = getFieldAndMethodChain(mapping, def, specs, element);
...
function generateFromFHIRAssignment(cardIsList, baseCardIsList, constraintsLength, fhirElement, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  ...
  if(fhirElementPath.split('.')[1] === 'extension'){
    ...
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      const shrMapping = shrElementMapping[shrElementMapping.length - 1];
      const shrType = fhirMappingToIdentifier(shrMapping).fqn;
      const dec = cardIsList && baseCardIsList ? 'const ' : ''; // if it's a list, we are declaring a variable
