...
function generateClass(def, fhir) {
  ...
  // TODO: createInstanceFromFHIR should be conditionally added. but the logic to check where it's necessary is basically all of writeFromFhir() so i don't want to dup that here
  const imports = ['setPropertiesFromJSON', 'createInstanceFromFHIR'];
...
function generateClassBody(def, fhir, cw) {
  ...
  cw.blComment( () => {
    cw.ln(`Deserializes FHIR JSON data to an instance of the ${clazzName} class.`)
      ...
      .ln(`@param {asExtension=false} Whether the provided instance is an extension`)
      .ln(`@returns {${clazzName}} An instance of ${clazzName} populated with the FHIR data`);
  }).bl('static fromFHIR(fhir, asExtension=false)', () => writeFromFhir(def, fhir, cw))
    .ln();
}
      ...
      if(mapping !== undefined){
        if(mapping.map === '<Value>'){
          ...
          if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
            generateFromFHIRAssignment(def.value.card.isList, baseIsList, 1, element, [], 'value', fhirProfile, null, cw);
          } else {
          ...
          if(fieldMapPath){
            ...
            if(field !== undefined){
              ...
              if (!alreadyMappedElements.has(classMethodChain)) {
                ...
                const slicing = sliceMap[element.path] && sliceMap[element.path][element.sliceName];
                generateFromFHIRAssignment(field.card.isList, baseIsList, constraintsLength, element, fieldMapPath, classMethodChain, fhirProfile, slicing, cw);
    ...
    cw.bl(`if (asExtension)`, () => {
      fhirExtension.differential.element.forEach( (element, i) => {
        if(element.path.startsWith('Extension.value') && element.path !== 'Extension.value[x]'){
          ...
          cw.ln(`inst.value = fhir['${name}'];`); // TODO: seems like this may need to be wrapped in a createInstanceFromFHIR
 ...
 * @param {CodeWriter} cw the CodeWriter that is writing the file for this element
 */
function generateFromFHIRAssignment(cardIsList, baseCardIsList, constraintsLength, fhirElement, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  ...
  cw.bl(`if (${check})`, () => {
      ...
      if(pathArray.length > 0){        
        cw.bl(`if(inst.${shrPathString} == null)`, () => {
          ...
          if (containerProfileArray && containerProfileArray.map(e => `['${e}']`).join('') == shrPathString) {
            ...
            // TODO: is there possibility of polymorphism issues here?
    ...
    if(fhirElementPath.split('.')[1] === 'extension'){
      ...
      cw.bl(`if (match != null)`, () => {
        // TODO -- ideally we would only build up the nested class structure above if we had a match on the extension
        ...
        if (slicing && slicing.length > 0) {
          ...
          sliceCriteria = slicing.map(d => {
            ...
            subPathArray.forEach(v => {
              prev = `${prev}['${v}']`;
              check = check + ' && ' + prev + ' != null'; // TODO: what if one of these was an array?
            });

            check = `${check} && ${prev} == '${d.value}'`; // TODO: what if d.value is a complex object like a codeableConcept?
