...
function generateClass(def) {
  ...
  cw.ln(`import jsonHelper from '${relativeImportPath(def.identifier, 'json-helper')}';`).ln();
...
function generateClassBody(def, cw) {
  ...
  cw.blComment( () => {
    ...
    cw.ln(`Deserializes JSON data to an instance of the ${clazzName} class.`)
      ...
      .ln(`@return {${clazzName}} An instance of ${clazzName} populated with the JSON data`);
  })
...
function writeFromJson(def, cw) {
  ...
  cw.ln('jsonHelper.setPropertiesFromJSON(inst, json);');
  cw.ln('return inst;');
}
 ...
 * @return {string} A relative path to where the imported class or file can be expected to be found
 * @private
 */
