 ...
 * @private
 */
export function createInstanceFromFHIR(key, value, shrId, allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
  if (Array.isArray(value)) {
    return value.map(v => createInstanceFromFHIR(key, v, shrId, allEntries, mappedResources, referencesOut, asExtension));
  }
  if (OBJECT_FACTORY == null) {
    throw new Error(`SHR ES6 module is not initialized.  Import 'init' before using the ES6 factories and classes`);
  }
 ...
 */
export function createReference(shrObject, referenceList=[], setReference=true) {
  if (!shrObject) {
    return null;
  }

  // this is kind of a hack but it makes the logic in the individual classes a lot cleaner
  // if we got a fake object with strings instead of full shr classes, wrap them in the right classes
  let shrId = shrObject.entryInfo.shrId;
  if (typeof shrId === 'string') {
    shrId = new ShrId().withValue(shrId);
  }

  let entryId = shrObject.entryInfo.entryId;
  if (typeof entryId === 'string') {
    entryId = new EntryId().withValue(entryId);
  }

  let entryType = shrObject.entryInfo.entryType;
  if (typeof entryType === 'string') {
    entryType = new EntryType().withValue(entryType);
  }

  const ref = new Reference(shrId, entryId, entryType);
  if (setReference) {
