...
function generateNamespaceFactory(ns, defs) {
  ...
  cw.ln(`import { getNamespaceAndName, getNamespaceAndNameFromFHIR } from '${relativeImportPath(ns.namespace, 'json-helper')}';`);
  ...
  cw.ln()
    ...
    .bl(`export default class ${factory}`, () => {
      ...
      cw.blComment(() => {
        ...
        .bl('static createInstanceFromFHIR(fhir, type, allEntries=[], mappedResources={}, referencesOut=[], asExtension=false)', () => {
          ...
          for (const def of defs) {
            const elName = className(def.identifier.name);
            cw.ln(`case '${elName}': return ${elName}.fromFHIR(fhir, allEntries, mappedResources, referencesOut, asExtension);`);
          }
          cw.ln(`default: throw new Error(\`Unsupported type in ${factory}: \${type}\`);`)
            .ln('}');
        });
