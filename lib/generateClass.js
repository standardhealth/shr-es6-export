...
function writeToFhir(def, fhir, cw) {
  ...
  if(fhirProfile !== undefined){
    ...
    //   addMapping('Patient.extension', {'identity': 'shr', 'map': '<shr.fhir.ComplexExension>'}, "Patient:shr-fhir-PatientEntry.extension:complexextension");
    // }
  ...
  } else if (def.identifier.isPrimitive){
    // debugger
  
  } else {

    let extension = fhir.extensions.find(p => p.id === fhirID(def.identifier, 'extension'));

    if(extension){
      cw.bl(`if (asExtension)`, () => {

        extension.differential.element.forEach( (element) => {
          if(def.identifier.name === 'StringValue'){
            //debugger
          }
    ...
    if (def.value) {
      ...
      cw.bl(`if (!asExtension && this.value != null)`, () => {
        ...
        // No profile, not an entry, but it has a value so we can resolve the value // This will recursively call toFHIR on the values until we reach a primitive
        cw.bl(`if (this.value != null)`, () => {
          if (def.value.card.isList) {
...
function generateFHIRAssignment(card, fhirElementPath, valueString, cw) {
  ...
  cw.bl(`if (${check})`, () => {
    ...
    if(fhirElementPath.split('.')[1] === 'extension'){
      // cw.ln(`inst['extension'] = this.${valueString}.toFHIR(true);`);
