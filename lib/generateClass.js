...
function generateClass(def, fhir) {
  ...
  cw.ln(`import { setPropertiesFromJSON, createInstanceFromFHIR } from '${relativeImportPath(def.identifier, 'json-helper')}';`).ln();
  ...
  // imports for referenced types
  // TODO: we don't need all of these, just the ones that have a nested value below them. figure out how to clean these up
  const referencedTypes = new Set();
  if (def.value) {
    if (def.value.identifier) {
      referencedTypes.add(`<${def.value.identifier.fqn}>`); // make it look like a 'fhir mapping'
    } else if (def.value.options) {
      def.value.options.forEach(o => referencedTypes.add(`<${o.identifier.fqn}>`));
    }
  }
  ...
  if (fhirProfile != null) {
    for (let element of fhirProfile.snapshot.element) {
      ...
      if(mapping != undefined && mapping.map !== '<Value>') { 
  ...
  Array.from(referencedTypes).sort().forEach( type => {
    const identifier = fhirMappingToIdentifier(type);
    if (!identifier.namespace // hack to make the tests work. some of the fixtures map to "<integer>" which we can't import
...
function writeFromFhir(def, fhir, cw) {
  ...
  if(fhirProfile != null){
    ...
    cw.bl(`if (fhir != null)`, () => {
      ...
      if (def.value.identifier != null && def.value.identifier.isPrimitive) {
        // we know it's a primitive value so we can set it directly
        cw.ln(`inst.value = fhir;`);
      } else {
        // it could be pretty much anything here, so just call createInstance to leverage that logic
        cw.ln(`inst.value = createInstanceFromFHIR('', fhir);`); // TODO: pick a key
      }        
    });
  }
  ...
  cw.bl(`if (${check})`, () => {
      ...
      if(pathArray.length > 0){        
        cw.bl(`if(inst.${shrPathString} === undefined)`, () => {
          ...
          if (containerProfileArray && containerProfileArray.map(e => `['${e}']`).join('') == shrPathString) {
            ...
            const shrType = fhirMappingToIdentifier(shrMapping).name;
    ...
    if(fhirElementPath.split('.')[1] === 'extension'){
      ...
      cw.bl(`if (match != null)`, () => {
          ...
          cw.ln(`inst.${shrElementPath} = createInstanceFromFHIR('${shrType}', match);`); // TODO: key
      });
      ...
      if (cardIsList) {
        ...
        cw.ln(`inst.${shrPathString} = createInstanceFromFHIR('${shrType}', fhir${fhirPathString}[0]);`); // TODO: key
        ...
        if (containerProfileArray) {
          ...
          if (shrElementPath == 'value') {
            ...
            const shrMapping = shrElementMapping[shrElementMapping.length - 1];
            const shrType = fhirMappingToIdentifier(shrMapping).fqn;
            cw.ln(`inst.${shrElementPath} = createInstanceFromFHIR('${shrType}', fhir${fhirPathString});`); // TODO: key
          }
        }
      }
    }
