...
function generateClass(def, specs, fhir) {
  ...
  cw.ln(`import { valueSet } from '${relativeImportPath(def.identifier, 'valueSets')}';`).ln(); // TODO: should be conditional
    ...
    for (let i = 0 ; i < pathArray.length ; i++) {
      ...
      if (i == pathArray.length - 1) {
        ...
        if (discriminator.value) {
          ...
          discCheck = `${discCheck} && valueSet('${discriminator.valueSet}').includes(${bracketNotation(prev)})`;
        }
      }
    }
  ...
  if (fhirElement.type[0].code === 'Reference') {
    ...
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      if (shrType.isPrimitive && !isExtension) {
        ...
        rhs = `createInstanceFromFHIR('${shrType.fqn}', ${fhirPathString}, shrId, allEntries, mappedResources, referencesOut, ${isExtension})`;
      }
      if (isRef) {
        rhs = `createReference( ${rhs}, referencesOut)`;
      }
