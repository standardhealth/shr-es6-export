const {DataElement, Value, IdentifiableValue, RefValue, ChoiceValue, TBD, INHERITED} = require('shr-models');
...
function generateClass(def) {
  ...
  cw.bl(`class ${clazzName}${superClass ? ` extends ${superClass}` : ''}`, () => {
    generateClassBody(def, cw, superClass != null);
  });
 ...
 * @param {boolean} hasSuperClass - whether or not the definition has a superclass.
 * @private
 */
function generateClassBody(def, cw, hasSuperClass) {
  cw.ln();
  ...
  cw.bl('function fromJSON(obj)', () => writeFromJson(def, cw, hasSuperClass));
}
 ...
 * @private
 */
function writeFromJson(def, cw, hasSuperClass) {
  cw.ln(`const ret = ${hasSuperClass ? 'super.fromJSON(obj)' : {}};`);
  if (def.value && def.value.inheritance != INHERITED) {
    cw.ln(`this.value = `)
  }
  for (const field of def.fields) {
