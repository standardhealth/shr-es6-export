 ...
 * @private
 */
export function createInstanceFromFHIR(key, value, allEntries=[], mappedResources={}, referencesOut=[], asExtension=false) {
  if (Array.isArray(value)) {
    return value.map(v => createInstanceFromFHIR(key, v), allEntries, mappedResources, referencesOut, asExtension);
  }
  if (typeof value === 'object') {
    if (OBJECT_FACTORY == null) {
      throw new Error(`SHR ES6 module is not initialized.  Import 'init' before using the ES6 factories and classes`);
    }
    return OBJECT_FACTORY.createInstanceFromFHIR(value, key, allEntries, mappedResources, referencesOut, asExtension);
  }
  return value;
}
 ...
 */
export function createReference(shrObject, referenceList=[]) {
  // TODO: what exactly are these fields?
  const shrId = '';
  const entryId = '';
  const entryType = '';
