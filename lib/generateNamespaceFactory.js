  ...
  cw.ln()
    ...
    .bl(`export default class ${factory}`, () => {
      ...
      cw.blComment(() => {
        ...
        .bl('static createInstanceFromFHIR(fhir, type, allEntries=[], mappedResources={}, asExtension=false)', () => {
          ...
          for (const def of defs) {
            const elName = className(def.identifier.name);
            cw.ln(`case '${elName}': return ${elName}.fromFHIR(fhir, allEntries, mappedResources, asExtension);`);
          }
          cw.ln(`default: throw new Error(\`Unsupported type in ${factory}: \${type}\`);`)
            .ln('}');
        });
