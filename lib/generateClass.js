  ...
  if (fhirElement.type[0].code === 'Reference') {
    ...
    cw.bl('if (!mappedResources[entryId])', () => {
      ...
      cw.bl('if (referencedEntry)', () => {
        ...
        cw.ln(`mappedResources[entryId] = createInstanceFromFHIR('${shrType}', referencedEntry['resource'], shrId, allEntries, mappedResources, referencesOut);`);
      });
    });
    ...
    if (isRef) {
      ...
      cw.bl(`if (mappedResources[entryId])`, () => {
        cw.ln(`${shrElementPath} = createReference(mappedResources[entryId], referencesOut);`);
      });
      cw.bl(`else`, () => {
        ...
        cw.ln(`${shrElementPath} = createReference({ entryInfo: { shrId, entryId, entryType } }, [], false);`);
      });
    } else {
