    ...
    for (const mapping of element.mapping) {
      ...
      if(mapping.map === '<Value>'){
        ...
        if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
          generateFromFHIRAssignment(def.value.card.isList, fhirIsList, 1, element, fhirElementPath, [], 'value', fhirProfile, null, cw);
        ...
        let shrElementPath = 'inst.' + mapping.classMethodChain; 
        ...
        generateFromFHIRAssignment(field.card.isList, fhirIsList, 0, element, fhirElementPath, mapping.fieldMapPath, shrElementPath, fhirProfile, slicing, cw);              
