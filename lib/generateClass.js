...
function getFieldAndMethodChain(mapping, def, specs, element) {
  ...
  if(fieldMapPath){
    ...
    for(const pathElement of fieldMapPath) {
      ...
      // The fields in currDef don't have any knowledge of constraints defined in the previous definition in the chain.
      // For example, if field A had a constraint with path B, then when we get the currDef for A, we need to apply the constraint on B.
      // If the field (now also represented by currDef) has any child constraints, merge them to the appropriate fields in currDef.
      if (field.constraintsFilter.child.hasConstraints) {
        // Clone currDef to itself so we don't modify the original definition
        currDef = currDef.clone();
        // For each child constraint, identify the corresponding field in currDef, modify the constraint path, and merge it.
        field.constraintsFilter.child.constraints.forEach(c => {
          const mergeConstraint = c.clone();
          if (mergeConstraint.path.length > 0) {
            // The field we want to look for in currDef is the first identifier in the path
            const identifier = mergeConstraint.path.shift();
            let mergeField = [currDef.value, ...currDef.fields].find(f => {
              return f && (identifier.equals(f.identifier) || matchesEffectiveIdentifierOrIncludesTypes(f, identifier));
            });
            if (!mergeField && currDef.value instanceof ChoiceValue) {
              mergeField = currDef.aggregateOptions.find(o => {
                return identifier.equals(o.identifier) || matchesEffectiveIdentifierOrIncludesTypes(o, identifier);
              });
            }
            if (mergeField) {
              // mergeConstraint.path was already appropriately modified by shifting the path
              mergeField.addConstraint(mergeConstraint);
            }
          } else if (mergeConstraint.onValue) {
            const mergeField = currDef.value;
            if (mergeField) {
              mergeConstraint.onValue = false;
              mergeField.addConstraint(mergeConstraint);
            }
          }
        });
 ...
 * NOTE: Does not work with ChoiceValues.  Those should be handled separately.
