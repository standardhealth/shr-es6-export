...
function preProcessChoiceFields(elements, specs, fhir) {
  return elements.map(element => {
    if (!element.path.endsWith('[x]')) {
      return element; // passthrough for non-choice elements
    }
...
function writeFromFhirProfile(def, specs, fhir, fhirProfile, cw) {
  ...
  // elementHierarchy contains objects of the form { element: {FHIR element}, didOpenBlock: <boolean>, fhirBasePath: <string> }

  const allFhirElements = preProcessChoiceFields(fhirProfile.snapshot.element, specs, fhir);

  if (fhirProfile.id === 'shr-procedure-DiagnosticImagingPerformed') debugger;
  for (const element of allFhirElements) {
    ...
    if (element.id === 'Observation:oncocore-CancerProgression.code.coding') debugger;

    let fhirElementPath = element.path;

    // look ahead to see if this or any "child" elements have an SHR mapping

    // use a set to avoid duplicates (TODO: are duplicates normal or a bug somewhere down the line?)
    const elementMappings = Array.from(  new Set(( element.mapping || []).filter(m => m.identity === 'shr').map(m => m.map) )  );

    const childElementHasMapping = allFhirElements.some(e => e.path != element.path && e.path.startsWith(element.path) && e.mapping && e.mapping.find(m => m.identity === 'shr'));
    ...
    if (elementMappings.length > 0 || childElementHasMapping) {
      ...
      const fhirDisplayPath = fhirElementPath.split('.').map((e, i) => i == 0 ? e : `['${e}']`).join('');
      // for FHIR, wrap the elements in the path after the first one in []
      // this is not strictly necessary, but I prefer it for consistency
      // (access FHIR by bracket notation ex fhir[abc], access SHR by dot notations ex inst.xyz )

      if (elementIsList) {
        const basePath = createVariableName(fhirElementPath);
        // if it's a list, create a for loop (including '|| []'' to avoid a null check)
        cw.ln(`for (const ${basePath} of ${fhirDisplayPath} || []) {`).indent();
        ...
        cw.ln(`if (${fhirDisplayPath} != null) {`).indent();
      }
    ...
    for (const mapping of elementMappings) {
      ...
      if(mapping === '<Value>'){
        ...
        if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
          generateFromFHIRAssignment(def.value.card.isList, false, 1, element, element.path, [], 'value', fhirProfile, null, cw);
        ...
        if(fieldMapPath){
          ...
          const {field, classMethodChain} = getFieldAndMethodChain(fieldMapPath, def, specs, element, mapping);
          ...
          if (field) {
            ...
            if (elementIsList && field.card.isList) {
            ...
            generateFromFHIRAssignment(field.card.isList, false, 0, element, fhirElementPath, fieldMapPath, shrElementPath, fhirProfile, slicing, cw);              
 ...
 */
function getFieldAndMethodChain(fieldMapPath, def, specs, element, mapping) {
  let field;
  let currDef = def;
...
function generateFromFHIRAssignment(cardIsList, baseCardIsList, constraintsLength, fhirElement, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  ...
  let fhirPathString = fhirPathArray.map((e,i) => i == 0 ? e : `['${e}']`).join('');  // pathstring contains fhir['fhirFirstLevel']['fhirSecondLevel']
  ...
  if(fhirElementPath.split('.')[1] === 'extension'){
    ...
    cw.ln(`${shrElementPath} = mappedResources[refID];`); // if it failed to map, this could be undefined, which is fine
  } else {
