      ...
      if(mapping.map === '<Value>'){
        ...
        for (let i = 0 ; i < mapping.fieldChain.length ; i++) {
          ...
          if (i == mapping.fieldChain.length - 1) { // if it's the last field in the chain
            ...
            cw.ln(`${dec}${shrElementPath} = ${nullCheck}createInstanceFromFHIR('${field.effectiveIdentifier.fqn}', {});`);
          }
    ...
    fhirExtension.differential.element.forEach( (element, i) => {
      if(element.path.startsWith('Extension.value') && element.path !== 'Extension.value[x]'){
        ...
        cw.bl(`if (${varName} != null)`, () => {
          cw.ln(`inst.${methodName} = createInstanceFromFHIR('${instance}', ${varName}, allEntries, mappedResources, true);`); // asExtension = true here, false(default value) everywhere else
        });
      }
    });
  });
  ...
  cw.bl(`if (!asExtension && fhir != null)`, () => {
    if (def.value.effectiveIdentifier != null) {
      if (def.value.effectiveIdentifier.isPrimitive) {
        ...
        cw.ln(`inst.value = createInstanceFromFHIR('${shrType}', fhir, allEntries, mappedResources);`);
      }
    } else {
      ...
      cw.ln(`inst.value = createInstanceFromFHIR(null, fhir, allEntries, mappedResources);`);
    }
  });
}
...
function generateFromFHIRAssignment(cardIsList, fhirElement, fhirElementPath, shrElementMapping, shrElementPath, fhirProfile, slicing, cw) {
  ...
  if(fhirElementPath.endsWith(`['extension']`)) {
    ...
    cw.bl(`if (match != null)`, () => {
      ...
      cw.ln(`${shrElementPath} = createInstanceFromFHIR('${shrType}', match, allEntries, mappedResources, true);`); // asExtension = true here, false(default value) everywhere else
    });
    ...
    cw.bl('if (!mappedResources[refID])', () => {
      ...
      cw.bl('if (referencedEntry)', () => {
        ...
        cw.ln(`mappedResources[refID] = createInstanceFromFHIR('${shrType}', referencedEntry['resource'], allEntries, mappedResources);`);
      });
    });
    ...
    if (shrElementPath == 'value' && shrElementMapping.length == 0) { // "primitive" values that do not map to an SHR type
      ...
      const shrMapping = shrElementMapping[shrElementMapping.length - 1];
      const shrType = fhirMappingToIdentifier(shrMapping).fqn;
      cw.ln(`${dec}${shrElementPath} = createInstanceFromFHIR('${shrType}', ${fhirPathString}, allEntries, mappedResources);`);
    }
  }
}
