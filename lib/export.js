...
class ES6Exporter {
  ...
  defToES6(def) {
    ...
    if (def.basedOn.length) {
      ...
      if (def.basedOn[0] instanceof TBD) {
        ...
        output.push(`class ${sanitizeIdentifierName(def.identifier.name)} extends ${def.basedOn[0].name} {\n`);
      }
    } else {
      output.push(`/** Generated from SHR definition for ${def.identifier.fqn} */\n`);
      output.push(`class ${sanitizeIdentifierName(def.identifier.name)} {\n`);
    }
    ...
    output.push(`\n}\n\n`);
    output.push(`export default ${sanitizeIdentifierName(def.identifier.name)};\n`);
    return output.join('');
  }
  ...
  fieldToGetterSetter(field) {
    ...
    if (field instanceof TBD) {
      ...
      const choiceSymbol = toSymbol(field.options.filter(o => !(o instanceof TBD)).map(o => sanitizeIdentifierName(o.identifier.name)).join('Or'));
      output.push(`  /**\n`)
