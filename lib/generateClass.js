...
function generateClassBody(def, cw) {
  ...
  cw.blComment( () => {
    ...
    cw.ln(`Serializes an instance of the ${clazzName} class to a JSON object.`)
      .ln(`The JSON should be valid against the ${clazzName} JSON schema, although this is not validated by the function.`)
      .ln(`@returns a JSON object populated with the data from the element`);
  })
...
function writeToJson(def, cw) {
  ...
  if (def.isEntry) {
    ...
    const url = `http://standardhealthrecord.org/spec/${def.identifier.namespace.replace('.', '/')}/${def.identifier.name}`;
  ...
  if (def.value !== undefined) {
    cw.bl('if (this.value !== undefined)', () => {
      if (def.value instanceof ChoiceValue) {
        ...
        // References get a special treatment too
        cw.bl(`if (this.${toSymbol(def.value.identifier.name)} !== undefined)`, () => {
          if (def.value.card.max > 1 || def.value.card.isMaxUnbounded) {
            cw.ln(`inst['Value'] = this.value.map(f => f.toJSON());`);
          } else {
  ...
  for (const field of def.fields) {
    ...
    // affect the definitions now, they may in the future, so we'll leave them in
    if (field.inheritance !== INHERITED) {
      cw.bl(`if (this.${toSymbol(field.identifier.name)} !== undefined)`, () => {
        if (field.card.max > 1 || field.card.isMaxUnbounded) {
          cw.ln(`inst['${field.effectiveIdentifier.fqn}'] = this.${toSymbol(field.identifier.name)}.map(f => f.toJSON());`);
        } else {
          cw.ln(`inst['${field.effectiveIdentifier.fqn}'] = this.${toSymbol(field.identifier.name)}.toJSON();`);
        }
      });
    }
  }
