          ...
          if(fieldMapPath){
            ...
            for(const pathElement of fieldMapPath) {
              ...
              if (elementIdentifier.isEntryKeyWord || elementIdentifier.isConceptKeyWord) {
                ...
                const fieldsPlusValue = [...currDef.fields];

                if (currDef.value instanceof IdentifiableValue) {
                  fieldsPlusValue.push(currDef.value);
                } else if (currDef.value instanceof ChoiceValue) {
                  fieldsPlusValue.push(...currDef.value.options);
                }

                field = fieldsPlusValue.find(f => elementIdentifier.equals(f.effectiveIdentifier));
              }
            ...
            if(field && field.card){ // TODO: figure out the bug where a couple instances have field.card == undefined
