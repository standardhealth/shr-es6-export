...
function writeToFhir(def, fhir, cw) {
  ...
  if(fhirProfile !== undefined){
   ...
   /* UNCOMMENT BELOW CODE TO JAM IN DUMMY MAPPING DATA */ 
    ...
    //   addMapping('Patient.deceasedBoolean', {'identity': 'shr', 'map': '<shr.fhir.Deceased>'});
    // }
    ...
    //   addMapping('Attachment.title', {'identity': 'shr', 'map': '<Value>'});
    // }
      ...
      if(mapping !== undefined){
        if(mapping.map === '<Value>'){
          if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
            generateAssignmentIfListFHIR(def.value.card, element.path, 'value', cw);
          } else {
          ...
          if(fieldMapPath){
            ...
            const field = def.fields.find(f => f.identifier.equals(fhirMappingToIdentifier(fieldMapPath[0]))); // getting cardinality from first in chain
            if(field !== undefined){
              generateAssignmentIfListFHIR(field.card, element.path, classMethodChain, cw);
            }
          }
        }
      }
...
function generateAssignmentIfListFHIR(card, fhirElementPath, valueString, cw) {
