...
describe('#FromFHIR_STU3', () => {
  ...
  describe.skip('#BloodPressureSliceByValueAndIncludesStrategyNestedComponents()', () => {
    ...
    before(() => {
      BloodPressureSliceByValueAndIncludesStrategyNestedComponents = context.importResult('shr/slicing/BloodPressureSliceByValueAndIncludesStrategyNestedComponents');
      EvaluationComponents = context.importResult('shr/slicing/EvaluationComponents');
      SystolicPressure = context.importResult('shr/slicing/SystolicPressure');
      DiastolicPressure = context.importResult('shr/slicing/DiastolicPressure');
      ComponentCode = context.importResult('shr/slicing/ComponentCode');
      Quantity = context.importResult('shr/core/Quantity');
    ...
    it('should deserialize a FHIR JSON instance', () => {
      const json = context.getFHIR('BloodPressureSliceByValueAndIncludesStrategy');
      const entry = BloodPressureSliceByValueAndIncludesStrategyNestedComponents.fromFHIR(json);
      expect(entry).instanceOf(BloodPressureSliceByValueAndIncludesStrategyNestedComponents);

      const expected = new BloodPressureSliceByValueAndIncludesStrategyNestedComponents()
        .withEvaluationComponents(
          new EvaluationComponents().withEvaluationComponent([
            new SystolicPressure()
              .withValue(
                new Quantity()
                  .withValue(120.0)
      ...
      fixExpectedEntryInfo(expected, 'http://standardhealthrecord.org/spec/shr/slicing/BloodPressureSliceByValueAndIncludesStrategyNestedComponents', entry, context);
