...
import { FHIR_GENERAL_PURPOSE_TYPES } from './fhirGeneralPurposeTypeMapping'
...
export const FHIRHelper = {
  ...
  /**
   * Identifies the type name for an object that is assumed to be of a FHIR general purpose type.
   * These objects do not have "resourceType" or anything like that to indicate what they are,
   * so we match type by what fields the given object has.
   * For example, a HumanName object has fields 'use', 'text', 'family', 'given', 'prefix', 'suffix', and 'period'.
   * If we get an object that has fields 'family' and 'given', we can safely assume it is a HumanName.
   * @param {Object} fhir - General purpose FHIR object
   * @returns {string} name of the FHIR type that this object is an instance of
   */
