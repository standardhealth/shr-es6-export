...
function writeFromFhir(def, fhir, cw) {
  ...
  if (!fhirProfile && def.value) {
    ...
    // If this can be resolved directly to a value, set the value directly
    cw.bl(`if (fhir != null)`, () => {
  ...
  cw.bl(`if (${check})`, () => {
    ...
    if(fhirElementPath.split('.')[1] === 'extension'){
      ...
      if (cardIsList) {
        ...
        cw.ln(`inst.${shrPathString} = inst.${shrPathString}.concat(fhir${fhirPathString}.map(f => ${shrType}.fromFHIR(fhir${fhirPathString})));`);
