...
function generateClassBody(def, fhir, cw) {
  ...
  cw.blComment( () => {
    ...
    .bl(`toFHIR()`, () => writeToFhir(def, fhir, cw));
}
...
function writeToFhir(def, fhir, cw) {
  ...
  const fhirProfile = fhir.profiles.find(p => p.id === fhirID(def.identifier));
  ...
  if(fhirProfile !== undefined){
   ...
   /* UNCOMMENT BELOW CODE TO JAM IN DUMMY MAPPING DATA */ 
    ...
    // const addMapping = (path, map) => { 
    //   const el = fhirProfile.snapshot.element.find(f => f.path === path);
    //   el.mapping = el.mapping.filter(m => m.identity !== 'shr');
    //   el.mapping.push(map);
    // };
    // if(fhirProfile.name === 'PatientConstraintsEntryProfile'){
    //   // addMapping('Patient.active', {'identity': 'shr', 'map': '<shr.fhir.ActiveFlag>'}); // DONE
    //   addMapping('Patient.birthDate', {'identity': 'shr', 'map': '<Value>'});
    //   addMapping('Patient.name.text', {'identity': 'shr', 'map': '<shr.fhir.Name>'});
    // }
    // if(fhirProfile.name === 'PractitionerEntryProfile'){
    //   addMapping('Practitioner.active', {'identity': 'shr', 'map': '<shr.fhir.IndirectActiveFlag>.<shr.fhir.ActiveFlag>'});
    // }
    ...
    for (element of fhirProfile.snapshot.element) {
      ...
      if(mapping !== undefined){
        // const match = mapping.map.match(/\<([^\>]*)>/);
        const match = mapping.map.match(/\<([^\>]*)>/g)
        if(match){
          const name = match.map(e => toSymbol(e.slice(1).split('.').pop().slice(0,-1))).join('.')
          // const name = match[1].split('.').pop();
          let fhirName = element.path.split('.');
          fhirName.shift();
          fhirName = fhirName.join('.');
          if(name === 'value'){
            if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
              generateAssignmentIfListFHIR(def.value.card, fhirName, name, cw);
            } else {
              console.error('ERROR: Value referenced in mapping but none exist on this element.');
            }
          } else {
            const field = def.fields.find(f => f.identifier.name === match[0].split('.').slice(-1)[0].slice(0,-1));
            generateAssignmentIfListFHIR(field.card, fhirName, name, cw);
          }
        }
      }
    }
  } else if (def.isEntry) {
    // TODO: Is an Entry, but no profile found.  This is a complete map and we must do something different

  } else if (def.value instanceof IdentifiableValue && def.value.identifier.isPrimitive) {
    cw.bl(`if (this.value != null)`, () => {
      cw.ln(`inst = this.value;`);
    });
  }
...
function generateAssignmentIfListFHIR(card, fhirPath, valueString, cw) {
  const valueArray = valueString.split('.')
  let prev = 'this.' + valueArray.shift();
  let check = `${prev} !== null`;
  ...
  cw.bl(`if (${check})`, () => {
    const path = fhirPath.split('.');
    let pathString = '';
    const previous = [];
    while(path.length > 0){
      previous.push(path.shift());
      pathString = previous.map(e => `['${e}']`).join('')
      if(path.length > 0){
    ...
    if (card.isList) {
      cw.ln(`inst${pathString} = this.${valueString}.map(f => f.toFHIR());`);
    } else {
